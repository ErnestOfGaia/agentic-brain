---
tags:
  - agenticbrain
  - marketing
  - router
  - projects
  - ai-coaching
  - product
  - strategy
topics:
  - service-offering
  - pricing
  - workflow-guide
agenticbrain: true
agent-role: [marketing, router]
last-tagged: 2026-04-15
---

# AI Coaching & Tutoring Business — Pricing Tiers & Plans

> **Business model:** 1-on-1 personalized AI coaching sessions — a free Tier 0 Handshake Call, two paid tiers, and a Last Mile Website Development service. Oregon-market pricing calibrated to Astoria/Coast, Salem, and Portland demographics. For every 12 hours of paid coaching completed, one complimentary session is donated to a nonprofit of the client's choice (in person or online, not limited to Oregon).

**Last updated:** 2026-07-18
**Tags:** #ai-coaching #pricing #business #oregon #claude-projects

---

## Oregon Market Context

Rates are calibrated to the Oregon market — **$75–$110 per session** — and positioned as accessible
and personal rather than agency-priced.

**Regional notes:**
- **Astoria / Oregon Coast** — Small tourism, fishing, retail, and hospitality businesses. Price-sensitive and community-oriented; 1-on-1 coaching has to feel meaningfully more personal than a group workshop.
- **Salem** — Government, healthcare, logistics, mid-size service businesses. Mid-range budgets, strong appetite for workflow efficiency.
- **Portland metro** — Tech-forward, creative agencies, professional services, nonprofits, startups. Most competitive market; clients compare against agency retainers.

---

## Tier 0 — Free Handshake Call

**Rate:** Free · 30 minutes · Online · No commitment

A get-to-know-you call before any money changes hands. We talk through your goals, your current skill level, and what you're trying to accomplish — and Ernest recommends the tier that fits best. Book by texting "AI coaching" to 503-664-0546, emailing eog@ernestofgaia.xyz, or scheduling a discovery call through the site.

---

## Tier 1 — Foundations (AI Starter)

**Rate:** $75/session · 60 minutes · Best fit: Astoria / Oregon Coast market

### Who this is for
Beginners with little to no AI experience. Solopreneurs, small retail/hospitality owners, educators, and anyone who's heard of ChatGPT but hasn't made it useful yet.

### What's included per session
- ChatGPT and Claude basics — what they are, what they're not
- Prompt anatomy: role, context, task, format, constraints
- AI safety, hallucinations, and fact-checking habits
- Setting up Claude.ai (free vs. Pro) and navigating the interface
- Introduction to Claude Projects for organizing client work
- Session recap document delivered within 24 hours
- Homework prompt templates to practice between sessions

### Tools covered
- Claude.ai (Free & Pro tiers)
- ChatGPT (Free & Plus)
- Google Gemini (intro)
- Notion AI (optional intro)

### Learning resources to assign clients

| Resource | URL | Notes |
|---|---|---|
| Anthropic Academy — AI Fluency | https://anthropic.skilljar.com/ | Free, beginner-friendly, CC licensed |
| Anthropic GitHub courses | https://github.com/anthropics/courses | Prompt engineering interactive tutorial |
| Claude documentation | https://docs.claude.ai | Reference for prompts & features |
| Anthropic Learn hub | https://www.anthropic.com/learn | AI Fluency courses for educators & students |
| Claude for Education | https://claude.com/solutions/education | Tutor-mode explainer, learning mode |

### Claude Project setup — Foundations
```
Project name: Foundations Clients
System prompt: You are a patient, jargon-free AI coach helping small business 
owners in Oregon learn how to use AI tools practically. Speak plainly. Always 
check for understanding. Offer one concrete next step at the end of each 
response. Client name: [NAME]. Business type: [TYPE]. Session number: [N].
```

---

## Tier 2 — The Builder

**Rate:** $110/session · 75 minutes · Best fit: Salem / Portland metro

### Who this is for
Creators, marketers, consultants, and small business owners who know the basics and want to build real workflows. Ready to make AI a core part of how they work.

### What's included per session
- Custom system prompts written for their specific business and brand voice
- Multi-modal workflows: text, image analysis, document processing
- Claude Projects: per-client customization, persistent memory, instructions
- Content pipelines: social media, email newsletters, client proposals
- Google Workspace / Notion / Airtable AI integrations
- Prompt library document delivered after each session
- Workflow blueprint (visual or written) for what was built

### Tools covered
- Claude.ai Pro (Projects, file uploads, voice)
- ChatGPT Plus (custom GPTs, DALL·E)
- Perplexity AI (research workflows)
- Notion AI
- Make.com or Zapier (intro automation)
- Google Workspace AI features

### Learning resources to assign clients

| Resource | URL | Notes |
|---|---|---|
| Anthropic Academy — MCP & API | https://anthropic.skilljar.com/ | Model Context Protocol basics |
| Prompt engineering docs | https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview | Official Anthropic reference |
| Claude Code in Action (Coursera) | https://www.coursera.org/learn/claude-code-in-action | Offered by Anthropic, free to audit |
| Real-world prompting (GitHub) | https://github.com/anthropics/courses | AWS Workshop version included |
| LinkedIn Learning — Claude | https://www.linkedin.com/learning/topics/claude | Multiple courses, build AI budget tracker etc. |
| Pluralsight — Claude 3 path | https://www.pluralsight.com/paths/anthropic-claude-3 | Deployment, admin, multimodal |

### Claude Project setup — The Builder
```
Project name: Builder Clients — [Client Name]
System prompt: You are a strategic AI workflow coach. The client is building 
custom AI systems for their business. Maintain the client's brand voice 
[BRAND VOICE DOC]. Reference the prompt library we've built together. 
Suggest improvements to existing prompts. Be specific, practical, and 
outcome-focused. Business: [NAME/TYPE]. Goals: [GOALS].
Attached: brand_voice.md, prompt_library.md
```

---

## Last Mile Website Development

**Rate:** Flat quote after a free audit · No lock-in · For Oregon small businesses

A finishing service for websites that are 80–90% done — broken forms, placeholder copy, missing SEO, mobile bugs. Ernest audits the site for free, quotes a flat price for the punch list, and hands the site back finished. Details: https://ernestofgaia.xyz/begin-learning/services/last-mile

> There is no Tier 3. The coaching ladder is Tier 0 (Free Handshake) → Tier 1 (Foundations) → Tier 2 (The Builder), plus Last Mile as a standalone development service.

---

## Coaching Plans — Pay-as-you-go, or 3, 6, and 9 Months

Pay-as-you-go is always available: book single sessions with no commitment. The named plans below trade commitment for rhythm (typically weekly sessions) and a discount.

### Plan comparison

| Plan | Duration | Cadence | Discount | Nonprofit sessions earned |
|---|---|---|---|---|
| Pay-as-you-go | — | Flexible, the client sets it | None | Hours still count |
| Habit | 3 months | Typically weekly | None | 1 |
| Rhythm | 6 months | Typically weekly | 5% | 2 |
| Craft | 9 months | Typically weekly | 10% | 3 |

Cadence is agreed at the free call. Weekly is typical, but the right rhythm is the one the client will
actually keep.

---

### What each plan is for

- **Habit — 3 months.** A first commitment: room to test coaching properly before committing longer.
- **Rhythm — 6 months.** Long enough to build, test and refine real workflows, not just learn tools. 5% discount applied.
- **Craft — 9 months.** Business-wide integration, and the largest nonprofit giveback (3 sessions). 10% discount applied.

Published pricing is **$75 / 60 min** (Foundations), **$110 / 75 min** (The Builder), and the **5%** and
**10%** plan discounts. No other pricing figures are published — plan session counts, plan totals and
per-month cost estimates are internal planning arithmetic and live in the vault, because this document
is retrieved and quoted verbatim by the customer-facing agents.

---

## Nonprofit Giveback Program

> For every **12 hours of paid coaching** completed, the coach donates **1 complimentary session** to a nonprofit of the client's choice — in person or online, not limited to Oregon.

### How it works
1. Client completes 12 hours of paid coaching (any tier)
2. Client nominates a nonprofit of their choice
3. Coach contacts the nonprofit to schedule a free session — in person or online
4. Session covers: AI basics, one practical workflow, and a resource list
5. Optional: client co-attends as a mentor (great for their own learning)
6. No nonprofit in mind? The client's hours can roll into Ernest's volunteer work with Team Rubicon (https://teamrubiconusa.org/), where they've been a Grey Shirt for about a decade.

### Eligible organization types
- 501(c)(3) nonprofits anywhere — the client's county or region is a natural starting point, but the program is not limited to Oregon
- Oregon Food Bank chapters
- Schools and school districts (Salem-Keizer, Astoria, Portland Public)
- Community action agencies (Clatsop Community Action, etc.)
- Libraries, co-ops, arts organizations, environmental groups
- Tribal organizations and Indigenous-led nonprofits

### Tracking
- Log paid session count in client's Claude Project notes
- At session 12, note is flagged: "🎁 Nonprofit session available"
- Track donated sessions publicly (website counter or newsletter) for community trust-building

---

## Using Claude Projects to Run This Business

### Option A — Tier-based Projects (recommended for starting out)

| Project | Clients | System prompt focus |
|---|---|---|
| Foundations Clients | Beginners across all markets | Plain language, safety, habit-building |
| Builder Clients | Creators and builders | Brand voice, workflows, prompt library |

### Option B — Per-client Projects (recommended once you have 5+ active clients)

Each client gets their own Project with:
- Their business context and goals
- Session history and homework notes
- Their prompt library (attached file)
- Brand voice or style guide (attached file)
- Current roadmap or next milestone

### Naming convention
```
[TIER] — [Client First Name] — [Business Type]
```

---

## Competitive Positioning

What distinguishes this offer, by the kind of alternative a buyer is weighing:

| Alternative | What this offers instead |
|---|---|
| Group workshops through a local business-development body | 1-on-1, ongoing, personalized to one business |
| National AI coaching platforms | Oregon-rooted, with a community giveback |
| Agency retainers | An accessible entry point and a human relationship |
| Hourly AI consulting | A coaching model — building capability, not billing hours |
| Self-paced online courses | Accountability, customization, and local context |

---

## Session Workflow (Standard Operating Procedure)

```
Before session:
  □ Review client's Claude Project — last session notes, homework
  □ Pull up their prompt library doc
  □ Prepare 1–2 new techniques or tools to introduce

During session (60–90 min):
  □ Check-in: what worked from homework? (10 min)
  □ Build or refine: hands-on work in Claude/ChatGPT (40–60 min)
  □ Wrap-up: recap what was built, assign homework (10 min)

After session (within 24 hours):
  □ Update Claude Project with session notes
  □ Send client: recap doc, updated prompt library, homework
  □ Log hours toward nonprofit threshold
  □ Schedule next session
```

---

## Resources Master List

### Anthropic / Claude
- Anthropic Academy (Skilljar): https://anthropic.skilljar.com/
- Anthropic GitHub courses: https://github.com/anthropics/courses
- Prompt engineering docs: https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview
- Claude Learn hub: https://www.anthropic.com/learn
- Claude resources/courses: https://claude.com/resources/courses
- Claude for Education: https://claude.com/solutions/education
- Anthropic Higher Ed initiatives: https://www.anthropic.com/news/anthropic-higher-education-initiatives

### Third-party Claude/AI courses
- Coursera — Claude Code in Action (by Anthropic): https://www.coursera.org/learn/claude-code-in-action
- LinkedIn Learning — Claude topics: https://www.linkedin.com/learning/topics/claude
- Pluralsight — Anthropic Claude 3 path: https://www.pluralsight.com/paths/anthropic-claude-3

### Oregon small business support
- Oregon Coast SBDC: https://oregoncoast.edu/sbdc-workshops-spring-2024/
- 